...
def get_bert_multi_model(ctx="cuda"):
    model_path = '/home/kizunasunhy/pytorch-bert-crf-ner/bert_multi/'
    with open(model_path + 'bert_config.json', mode='r') as io:
...
class KobertOnly(nn.Module):
    """ koBERT alone """
    def __init__(self, config, num_classes) -> None:
        super(KobertOnly, self).__init__()
        self.name = self.__class__.__name__
    ...
    def forward(self, input_ids, token_type_ids=None, tags=None):
        ...
        '''
        if tags is not None:
            loss = self.criterion(tag_out, tags)
            return loss
        else:
            return tag_out
        '''
